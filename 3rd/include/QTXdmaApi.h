...
#define OFFSET_ADC_CTRL_EXTERNAL_REFERENCE                                                                             \
    0x04 // 外参考,外时钟使能 bit0=外时钟使能  bit1=外参考使能  高电平有效 二者互斥
#define OFFSET_ADC_CTRL_ADC_ENABLE 0x08 // 配置adc使能(上升沿有效)

#define OFFSET_BDINFO_BDINFO                                                                                           \
    0x00 // 板卡信息(子板,载板) 如0x7135_7010(无子板默认为载板信息) bit[31:16]:子板信息 bit[15:0]:载板信息
#define OFFSET_BDINFO_SOFT_VER 0x04 // 软件版本(年,月,日) 如0x2019_1208  bit[31:16]:年 bit[15:8]:月 bit[7:0]日
#define OFFSET_BDINFO_XDMA                                                                                             \
    0x08 // xdma信息(模式,链路通道速率,位宽) 如0x0808_0256表示x8模式 8Gb链路通道速率 256bit数据宽度
#define OFFSET_BDINFO_ADC                                                                                              \
    0x0C // adc采样率,通道数,分辨率 如0x2000_0416 bit[31:16]:adc采样率(单位M) bit[15:8]:adc通道数 bit[7:0]adc分辨率
#define OFFSET_BDINFO_DAC                                                                                              \
    0x10 // dac采样率,通道数,分辨率 如0x2500_0816 bit[31:16]:dac采样率(单位M) bit[15:8]:dac通道数 bit[7:0]dac分辨率
...
enum DIRECTION { DIRECTION_CARD2HOST, DIRECTION_HOST2CARD };
...
// value：寄存器写入值
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaWriteRegister(STXDMA_CARDINFO *pstCardInfo, uint64_t BaseAddr, unsigned int Offset,
                                      uint64_t Value);
...
// value：寄存器返回值
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaReadRegister(STXDMA_CARDINFO *pstCardInfo, uint64_t BaseAddr, unsigned int Offset,
                                     uint64_t *Value);
...
// unTimeOut，获取数据超时时间
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaGetDataBuffer(uint64_t unOffsetAddr, STXDMA_CARDINFO *pstCardInfo, unsigned char *pBufDest,
                                      unsigned int unLen, unsigned int unTimeOut);

// 函数功能: 关闭板卡
// 函数参数：无
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaGetData(uint64_t unOffsetAddr, STXDMA_CARDINFO *pstCardInfo, unsigned char *pBufDest,
                                unsigned int unLen, unsigned int unTimeOut);
...
// unTimeOut：写入数据超时时间
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaSendData(uint64_t unOffsetAddr, STXDMA_CARDINFO *pstCardInfo, void *pBufsrc, uint64_t unLen,
...
// unTrigCount：触发次数
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaSetInternalPulseTrigger(STXDMA_CARDINFO *pstCardInfo, uint32_t unPulsePeriod,
                                                uint32_t unPulseWidth, uint32_t unTrigCount);
...
// unTrigCount：触发次数
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaSetChannelTrigger(STXDMA_CARDINFO *pstCardInfo, int mode, int channelID,
                                          uint32_t unRisingEdgeTrigLevel, uint32_t unFallingEdgeTrigLevel,
                                          uint32_t unTrigCount);
...
// 函数返回: 成功返回0,失败返回-1并把详细错误信息写入日志文件
QTXDMAAPI_API int QTXdmaDataSplitChannels(char *MultiChannelFileName, int ChannelCount, int ChannelIndex,
                                          char *SingleChannelFileName);


QTXDMAAPI_API int QTXdmaSetParameterADC(
        ...
        uint32_t
                DMA_TransmitCount // dma0 传输的次数 连续模式下,传输次数完成后,切换基地址继续传输,直到不使能连续传输模式
);
QTXDMAAPI_API int QTXdmaSetParameterDAC(STXDMA_CARDINFO *pstCardInfo, uint32_t dacSampleRate,
                                        uint32_t enableExtRefClock, bool bEnableDAC, uint32_t ChannelCount,
                                        uint32_t ChannelID, uint32_t DMA_TransmitMode, uint32_t DMA_TransmitOnceBytes,
...
QTXDMAAPI_API int QTXdmaShow_file_content(char *filename, uint32_t dataCount, int StartEnd, int offsetBytes,
                                          int dataType);
